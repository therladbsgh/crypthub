                ...
                <Table.Body>
                    {_.map(transactions, (t, index) => 
                        <Table.Row key={index} positive={t.filled} error={!current && !t.filled && t.expiration <= now}>
                            ...
                            <Table.Cell>{date.format(new Date(t.date), 'MM/DD/YYYY')}</Table.Cell>
                            <Table.Cell>{t.symbol}</Table.Cell>
                            <Table.Cell>{t.size}</Table.Cell>
                            <Table.Cell>{formatCurrency(t.price, { format: '%s%v', symbol: '$' })}</Table.Cell>
                            <Table.Cell>{current ? t.GTC ? 'GTC' : `Expires in ${date.subtractStr(new Date(t.expiration), now)}` : t.filled ? `Filled ${date.format(new Date(t.filledDate), 'MM/DD/YYYY')}` : `Expired ${date.format(new Date(t.expiration), 'MM/DD/YYYY')}`}</Table.Cell>
                        </Table.Row>
                    )}
                </Table.Body>
            </Table>
