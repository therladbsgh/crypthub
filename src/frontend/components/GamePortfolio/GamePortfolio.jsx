...
export default class GamePortfolio extends Component {
	...
	render() {
		const { gameId, player, completed, updateGame } = this.props;
		...
		return (
			<div>
				...
				<Transactions gameId={gameId} playerId={player._id} transactions={history ? player.transactionHistory : player.transactionCurrent} current={!history} updateGame={updateGame} />
			</div>
		);
	}
}
